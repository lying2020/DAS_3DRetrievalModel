...
function [xcorrArray, timeLag, fig] = crosscorrelation(arr1, arr2, maxLag)
% -----------------------------------------------------------------------------------------------------
...
% ex: timeLag = 2, shows arr1 being delayed with respect to arr2 by two samples.                
...
if (length(arr1) - length(arr2)), opt = 'none';   end
% -----------------------------------------------------------------------------------------------------
% Matlabµ÷ÓÃxcorr¼ÆËã
% scaleopt ¡ª Normalization option
% 'none' (default) | 'biased' | 'unbiased' | 'normalized' | 'coeff'
[xcorrArray, lag] = xcorr(arr1, arr2, maxLag, opt);   %  'biased'  %  'unbiased'
