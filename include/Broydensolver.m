function Result = Broydensolver(layerCoeffModel,layerGridModel,Ft,Fun,DFun,initialX,Toler,Maxnumiter)
%%INPUT:
...
X1(3) = layerz_tanyan(layerCoeffModel,layerGridModel,X1([1,2])',1);
if smallt > Ft(X1)
    smallt = Ft(X1);
    smallX = X1;
end
...
for j=1:Maxnumiter
    ...
    if acstep > 10
        ...
        acstep = acstep + norm(step);
        X2(3) = layerz_tanyan(layerCoeffModel,layerGridModel,X2([1,2])',1);
        if smallt > Ft(X2)
            smallt = Ft(X2);
            smallX = X2;
        end
