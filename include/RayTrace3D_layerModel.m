function  [Position, markX0 ,initialguessVel,errort,trivalt] = RayTrace3D_layerModel(layerCoeffModel, layerGridModel, velocityModel, sensorCoord, sourceCoord)
%% Describe:
...
if k <= 2
    ...
    deleteX0 = [];
    if norm(X0(1:3,k) - X0(1:3,k-1)) < inter/10
        deleteX0(end+1) = k-1;
    end
    if norm(X0(1:3,2) - X0(1:3,1)) < inter/10
        deleteX0(end+1) = 2;
    end
    for iX0 = k-1:-1:3
