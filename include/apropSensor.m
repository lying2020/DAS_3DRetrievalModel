...
idx = (1:numLayer)';
z = [layerz_tanyan(layerCoeffModel, layerGridModel,xyArray,idx);inversionLocation(1,3)];
z = sortrows(z,1);
...
for iou = 2:2
    ...
    for iSensor = 1:numSensor
        if row == 1
            if  Sensor(iSensor,3) < layerz_tanyan(layerCoeffModel(row),layerGridModel(row),Sensor(iSensor,1:2),1)+tol
                DSensor(end+1) = iSensor;
            end
        elseif row == numLayer+1 || row == numLayer
            %         if Sensor(iSensor,3) > layerz_tanyan(layerCoeffModel(row-1),layerGridModel(row-1),Sensor(iSensor,1:2),1)-tol
            %             DSensor(end+1) = iSensor;
            %         end
        else
            if  Sensor(iSensor,3) < layerz_tanyan(layerCoeffModel(row),layerGridModel(row),Sensor(iSensor,1:2),1)+100 ...
                    && Sensor(iSensor,3) > layerz_tanyan(layerCoeffModel(row-1),layerGridModel(row-1),Sensor(iSensor,1:2),1)-100
                DSensor(end+1) = iSensor;
            end
        end
    end
